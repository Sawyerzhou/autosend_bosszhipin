...
ActionChains(wd).move_to_element(clickable).perform()
...
# 进入首页
hoverable = WebDriverWait(wd, 45).until(
...
while i:
    try:
        search_input = WebDriverWait(wd, 45).until(EC.element_to_be_clickable((
...
while i:
    try:
        # 等待搜索结果加载
        WebDriverWait(wd, 45).until(EC.text_to_be_present_in_element_attribute((
...
while var == 1:
    ...
    for job in jobs:
        ...
        # 操作滚轮到当前职位可见
        ActionChains(wd).scroll_to_element(job).perform()
        random_sleep(1, 2)
        # 打开职位详情页
        ...
        # 等待沟通按钮可见
        try:
            button = WebDriverWait(wd, 45).until(
            ...
            if b_text == "立即沟通":
                ...
                # 等待关闭按钮可见
                try:
                    close_button = WebDriverWait(wd, 45).until(
    ...
    # 切换下一页
    try:
        last_button = WebDriverWait(wd, 45).until(
